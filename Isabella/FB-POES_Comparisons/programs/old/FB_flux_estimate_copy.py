...
def midpoints(colArray):
    ...
    while n < 98:
        ...
        n += 1
    #print('this is midpointArray')
    #print(midpointArray)
    print('this is colwidtharray')
    print(colwidthArray)
    return midpointArray, colwidthArray

def calculate_counts(colwidthArray, GfactorList, J_flux):
    newcountArray = np.zeros((97), dtype=float)
    cadence = 0.05
    GfactorArray = np.array(GfactorList, dtype=float)
    print('print Gfactor shape')
    print(GfactorArray.shape)
    
    newcountArraySum = 0
    n = 0
    while n < 97:
        newcountArray[n] = J_flux[n] * cadence * GfactorArray[n, 0] * (colwidthArray[n] * 1000.)
        newcountArraySum = newcountArraySum + newcountArray[n]
        n += 1     
